...
function M.CompareAction(
    ...
    for l in res:gmatch("[^\r\n]+") do
        ...
        mark_id.code[codeln] =
            vim.api.nvim_buf_set_extmark(bufnr, extmark.code, end_line + codeln + 1 + offset, 0,
                extmark_opts.code[codeln])
        codeln = codeln + 1
    end

    -- create line to display LLM separator virtual text
    F.InsertTextLine(bufnr, end_line + codeln + 1 + offset, "")
    mark_id["llm"] = vim.api.nvim_buf_set_extmark(bufnr, extmark.llm, end_line + codeln + 1 + offset, 0, extmark_opts
    .llm)
    return codeln
end
...
function M.qa_ch2en_handler(name, F, state, streaming, prompt, opts)
    if prompt == nil then
...
function M.Template_interact_on_selected_handler(name, _, state, streaming, prompt, title, width, height)
    ...
    local layout = F.CreateLayout(width.."%", height.."%",
        Layout.Box({ Layout.Box(input_box, { size = "15%" }),
...
function M.Template_preview_on_selected_handler(name, _, state, streaming, prompt, title, width, height)
    ...
    local layout = F.CreateLayout(width .. "%", height .. "%", Layout.Box(
        { Layout.Box(preview_box, {
            size = "90%"
        }) }, {
            dir = "col"
        }))
