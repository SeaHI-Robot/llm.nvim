...
function M.SetInput(bufnr, winid)
  ...
  for k, v in pairs(conf.configs.keys) do
    ...
    elseif conf.configs.save_session and k == "Input:HistoryNext-n" and conf.configs.output_box_opts.style == "float" then
      F.WinMapping(input_popup, v.mode, v.key, function()
        F.MoveHistoryCursor(1)
      end, { noremap = true })
    elseif conf.configs.save_session and k == "Input:HistoryNext-i" and conf.configs.output_box_opts.style == "float" then
      F.WinMapping(input_popup, v.mode, v.key, function()
        F.MoveHistoryCursor(1)
      end, { noremap = true })
    elseif conf.configs.save_session and k == "Input:HistoryPrev-n" and conf.configs.output_box_opts.style == "float" then
      F.WinMapping(input_popup, v.mode, v.key, function()
        F.MoveHistoryCursor(-1)
      end, { noremap = true })
    elseif conf.configs.save_session and k == "Input:HistoryPrev-i" and conf.configs.output_box_opts.style == "float" then
      F.WinMapping(input_popup, v.mode, v.key, function()
        F.MoveHistoryCursor(-1)
      end, { noremap = true })
    end
