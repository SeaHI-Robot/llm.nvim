        ...
        if conf.configs.output_box_opts.style == "float" then
            ...
            for k, v in pairs(conf.configs.keys) do
                ...
                elseif k == "Session:Toggle" then
                    F.WinMapping(llm_popup, v.mode, v.key, F.ToggleLLM, { noremap = false })
            ...
            for k, v in pairs(conf.configs.keys) do
                ...
                elseif conf.configs.save_session and k == "Output:HistoryPrev" and conf.configs.output_box_opts.style == "float" then
                    vim.keymap.set({ "n", "i", "v" }, v.key, function() F.MoveHistoryCursor(-1) end,
                        { buffer = bufnr, noremap = true, silent = true })
                elseif conf.configs.save_session and k == "Output:HistoryNext" and conf.configs.output_box_opts.style == "float" then
                    vim.keymap.set({ "n", "i", "v" }, v.key, function() F.MoveHistoryCursor(1) end,
                        { buffer = bufnr, noremap = true, silent = true })
                end
            end
